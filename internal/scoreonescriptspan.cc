...
void ScoreEntireScriptSpan(const LangSpan& scriptspan,
  ...
  if (scoringcontext->flags_cld2_html) {
    ChunkSummary chunksummary = {
      1, 0,
      static_cast<uint16_t>(one_one_lang) , UNKNOWN_LANGUAGE, static_cast<uint16_t >(score), 1,
      static_cast<uint16_t>(bytes), 0, static_cast<uint16_t>(scriptspan.ulscript), static_cast<uint8_t>(reliability), static_cast<uint8_t>(reliability)
    };
